...
To integrate from C++ or Python you must use the InOrbit Agent Core. It can be installed in different ways, including via a [Debian package](https://www.inorbit.ai/docs#debian-installer). For simplicity in this documentation a one-line command that downloads and executes the agent installer is used. After installing the agent you'll be able to start seeing data from your robot, such as CPU usage, when using InOrbit.
